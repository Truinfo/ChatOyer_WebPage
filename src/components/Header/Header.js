...
const Header = () => {
  ...
  const renderSuggestions = () => {
    ...
    return (
      ...
      <Dropdown.Menu show={limitedSuggestions.length > 0} className="suggestions-dropdown"  style={{ width: "300px", left: 130, transform: "translateX(300%) translateY(0%)" }}>
        {limitedSuggestions.map((suggestion) => (
          <Dropdown.Item key={suggestion._id} onClick={() => handleSuggestionClick(suggestion)} className="text-dark fs-4">
              {suggestion.name}
          </Dropdown.Item>
        ))}
      </Dropdown.Menu>
              ...
              <div className="menu p-2 ">
                <Dropdown show={showUserDropdown} onToggle={toggleUserDropdown}>
                  <Dropdown.Toggle variant="light" id="user-dropdown" className="header-buttons p-2">
                    ...
                    {auth ? (
                    <Dropdown.Menu className="drop-menu fs-2" 
                    style={{ width: "100px", right: 50, transform: "translateX(-70%) translateY(10%)" }}>
