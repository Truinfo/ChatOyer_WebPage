...
const Cart = () => {
  ...
  const [myselectedSize, setSelectedSize] = useState(null);
  const [myselectedDiamondType, setSelectedDiamondType] = useState(null);
  const [myselectedGoldKt, setSelectedGoldKt] = useState(null);
  // const [myselectedGoldType, setSelectedGoldtype] = useState(null);
  ...
  const handleUpdate = async (
    ...
    try {
      ...
      const newQuantity =
        action === "increase" ? quantity + 1 : Math.max(quantity - 1, 1);
      const finalTotal = myfinalTotal[productId];
      const response = await axiosInstance.put("/updateCartItem", {
        cartItemId,
        quantity: newQuantity,
        selectedGoldType: myselectedGoldType,
        selectedSize: myselectedSize,
        selectedDiamondType: myselectedDiamondType,
        selectedGoldKt: myselectedGoldKt,
        sizePrice: mysizePrice,
      ...
      if (response.status === 200) {
        ...
        fetchProduct(); // Fetch updated cart data after successful update
      } else {
